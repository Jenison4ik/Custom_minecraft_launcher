...
export default function AnimatedFileUpload({
  ...
  hint,
  idleLabel = "Drag & drop or click to upload",
  inputId,
  inputLabel,
  ...
  const validateFiles = useCallback(
    (newFiles: File[]): File[] => {
      ...
      if (accept) {
        const allowed = newFiles.filter((file) => matchesAccept(file, accept));
        if (allowed.length !== newFiles.length) {
          setError("Этот тип файла нельзя загрузить");
          return allowed;
        }
      }
      return newFiles;
  ...
  return (
    <div className={cn("w-full space-y-3", className)}>
      ...
      <input
        accept={accept}
        aria-hidden={inputLabel ? undefined : true}
        className="sr-only"
        disabled={disabled}
        id={inputId}
        multiple={multiple}
      ...
      >
        ...
        <AnimatePresence initial={false} mode="wait">
          ...
          >
            {isDragOver ? dropLabel : idleLabel}
          </motion.p>
        </AnimatePresence>
        <p className="mt-1 text-muted-foreground text-xs">
          {hint ?? (accept ? accept.replace(/,/g, ", ") : "Any file type")}
          {maxSize && !hint ? ` Max ${formatFileSize(maxSize)}` : null}
        </p>
      </motion.div>
